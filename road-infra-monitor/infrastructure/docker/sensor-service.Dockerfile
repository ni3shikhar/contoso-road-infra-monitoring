...
RUN --mount=type=cache,target=/root/.m2/repository \
    mvn dependency:go-offline -pl sensor-service -am -B \
    -Dmaven.resolver.transport=wagon \
    -Dmaven.wagon.http.ssl.insecure=true \
    -Dmaven.wagon.http.ssl.allowall=true

COPY backend/common-lib/src common-lib/src/
COPY backend/sensor-service/src sensor-service/src/

RUN --mount=type=cache,target=/root/.m2/repository \
    mvn package -pl sensor-service -am -DskipTests -Dmaven.test.skip=true -B \
    -Dmaven.resolver.transport=wagon \
    -Dmaven.wagon.http.ssl.insecure=true \
    -Dmaven.wagon.http.ssl.allowall=true
